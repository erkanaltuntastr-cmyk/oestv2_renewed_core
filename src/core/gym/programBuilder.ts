...
const clampCount = (items: Movement[], min: number, max: number): Movement[] => {
  if (items.length === 0) return [];
  const desired = Math.min(max, Math.max(min, items.length));
  return items.slice(0, desired);
};

const basePrescriptions: Record<Movement['category'], { sets: number; reps: number; weight: number | null }> = {
  warmup: { sets: 2, reps: 10, weight: null },
  main: { sets: 3, reps: 10, weight: null },
  optional: { sets: 2, reps: 12, weight: null },
};

export type PrescribedMovement = Movement & {
  prescribedSets: number;
  prescribedReps: number;
  prescribedWeight: number | null;
};

export const applyProgressiveOverload = (movement: Movement, dayIndex: number = 0): PrescribedMovement => {
...
export const buildDailyProgram = (user: Record<string, unknown> = {}, dayIndex: number = 0): DailyProgram => {
  ...
  const buildWithPrescription = (list: Movement[]): PrescribedMovement[] =>
    list.map((movement) => applyProgressiveOverload(movement, dayIndex));
