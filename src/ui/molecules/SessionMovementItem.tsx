...
export interface SessionMovementItemProps {
  movement: WorkoutMovement | null;
  onUpdate?: (id: number, data: Partial<WorkoutMovement>) => void;
  onRemove?: (id: number) => void;
}

export const SessionMovementItem: React.FC<SessionMovementItemProps> = memo(
  ({ movement, onUpdate, onRemove }) => {
    const handleChange = useCallback(
      (key: keyof Pick<WorkoutMovement, 'sets' | 'reps' | 'weight'>) =>
        (e: React.ChangeEvent<HTMLInputElement>) => {
          if (!movement) return;
          const value = Number(e.target.value) || 0;
          onUpdate?.(movement.id, { [key]: key === 'weight' ? value : value } as Partial<WorkoutMovement>);
        },
      [movement, onUpdate],
    );
...
SessionMovementItem.displayName = 'SessionMovementItem';
